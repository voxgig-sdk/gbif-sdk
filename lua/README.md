...
The Lua SDK for the Gbif API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Enumeration(nil):list(nil, nil)
...
```lua
local result, err = client:Enumeration(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Gbif(nil):load(
  { id = "test01" }, nil
)
