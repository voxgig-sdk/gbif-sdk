...
describe("EnumerationDirect", function()
  it("should direct-list-enumeration", function()
    local setup = enumeration_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-enumeration", setup.live and "live" or "unit")
  ...
  it("should direct-load-enumeration", function()
    local setup = enumeration_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-enumeration", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function enumeration_direct_setup(mockres)
