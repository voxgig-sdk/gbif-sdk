...
local OccurrenceEntity = {}
OccurrenceEntity.__index = OccurrenceEntity


function OccurrenceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OccurrenceEntity)
  self._name = "occurrence"
  self._client = client
...
function OccurrenceEntity:get_name()
  return self._name
end


function OccurrenceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OccurrenceEntity.new(self._client, opts)
end
...
function OccurrenceEntity:mark_deleted()
  self._deleted = true
end


function OccurrenceEntity:deleted()
  return true == self._deleted
end


function OccurrenceEntity:data_set(args)
  if args ~= nil then
...
function OccurrenceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OccurrenceEntity:match_set(args)
  if args ~= nil then
...
function OccurrenceEntity:match_get()
...
function OccurrenceEntity:stream(action, args, callopts)
...
function OccurrenceEntity:_run_op(ctx, post_done)
  local utility = self._utility
