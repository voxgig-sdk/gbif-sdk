...
local RegistryEntity = {}
RegistryEntity.__index = RegistryEntity


function RegistryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RegistryEntity)
  self._name = "registry"
  self._client = client
...
function RegistryEntity:get_name()
  return self._name
end


function RegistryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RegistryEntity.new(self._client, opts)
end
...
function RegistryEntity:mark_deleted()
  self._deleted = true
end


function RegistryEntity:deleted()
  return true == self._deleted
end


function RegistryEntity:data_set(args)
  if args ~= nil then
...
function RegistryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RegistryEntity:match_set(args)
  if args ~= nil then
...
function RegistryEntity:match_get()
...
function RegistryEntity:stream(action, args, callopts)
...
function RegistryEntity:_run_op(ctx, post_done)
  local utility = self._utility
