...
MCP server exposing the Gbif SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `gbif_list` | `entity`, optional `query` map | First page of records as JSON |
| `gbif_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `gbif_list` and
`gbif_load` tools become available in new sessions.
