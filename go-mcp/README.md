...
[MCP](https://modelcontextprotocol.io) server exposing the Gbif SDK as
two agent tools — `gbif_list` and `gbif_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gbif-mcp)
make build

# 2. Provide credentials via the environment
export GBIF_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user gbif \
  -- /absolute/path/to/gbif-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./gbif-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// gbif_list: first page of records
{ "entity": "enumeration" }
{ "entity": "enumeration", "query": { } }

// gbif_load: one record by id
{ "entity": "enumeration", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gbif-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user gbif \
     -- "$PWD"/dist/*/gbif-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `gbif_list` and `gbif_load` tools now appear
   in new sessions. Ask the agent to *"list enumeration using gbif"*
   and it calls `gbif_list` with `{"entity":"enumeration"}`.
...
```sh
export GBIF_APIKEY=sk_live_xxx            # API key
export GBIF_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `gbif_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `gbif_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GBIF_APIKEY` | API key sent with every request. |
| `GBIF_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

enumeration | literature | occurrence | registry | species | vocabulary

### Smoke test via HTTP (raw JSON-RPC)

```sh
./gbif-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gbif_load","arguments":{"entity":"enumeration","query":{"id":1}}}}'
```
