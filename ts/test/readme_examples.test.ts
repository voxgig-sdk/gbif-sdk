...
//     (\`new GbifSDK(...)\` / \`GbifSDK.test(...)\` become
//     \`GbifSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"enumeration":{"test01":{"id":"test01"}},"literature":{"test01":{"id":"test01"}},"occurrence":{"test01":{"id":"test01"}},"registry":{"test01":{"id":"test01"}},"species":{"test01":{"id":"test01"}},"vocabulary":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GbifSDK(...)\` and any
// \`GbifSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GbifSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GbifSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
