...
describe('EnumerationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'enumeration.' + op, live)) return
    }
    ...
    const enumeration_ref01_list = await enumeration_ref01_ent.list(enumeration_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GBIF_TEST_ENUMERATION_ENTID']
