...
describe('EnumerationDirect', async () => {
  ...
  test('direct-load-enumeration', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-enumeration', setup.live)) return
    if (skipIfMissingIds(t, setup, ["enumeration01"])) return
  ...
  test('direct-list-enumeration', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-enumeration', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GBIF_TEST_ENUMERATION_ENTID']
