...
(0, node_test_1.describe)('EnumerationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-enumeration', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-enumeration', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["enumeration01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.enumeration ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.enumeration = candidateId;
            params.enumeration = setup.idmap['enumeration01'];
        }
        else {
            params.enumeration = 'direct01';
        }
        const result = await client.direct({
            path: 'enumeration/basic/{enumeration}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-enumeration', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-enumeration', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.GBIF_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.GbifSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.GBIF_APIKEY,
            secret: env.GBIF_SECRET,
        }));
        let idmap = env['GBIF_TEST_ENUMERATION_ENTID'];
...
//# sourceMappingURL=EnumerationDirect.test.js.map