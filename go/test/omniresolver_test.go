...
import (
	...
	omni "github.com/voxgig-sdk/gbif-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GbifError); is {
				out := map[string]any{
					"name":    "GbifError",
...
func omniCtx(arg any, client *sdk.GbifSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GbifSDK); is {
			client = live
