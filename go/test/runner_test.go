...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("GBIF_TEST_LIVE") == "TRUE" ||
		os.Getenv("GBIF_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("GBIF_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.GbifSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.GbifError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.GbifSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.GbifError{Msg: msg, Code: code}
}
