...
class OccurrenceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OccurrenceEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Occurrence]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Occurrence().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: OccurrenceCreateData, ctrl=None) -> Occurrence:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
