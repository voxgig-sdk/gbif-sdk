...
class TestRegistryDirect:
    ...
    def test_should_direct_list_registry(self):
        setup = _registry_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-registry", "live" if setup["live"] else "unit")
        if _skip:
...
def _registry_direct_setup(mockres):
