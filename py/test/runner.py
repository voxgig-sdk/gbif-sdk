...
class GbifTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GbifTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GbifTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GbifTestRunner.getenv("GBIF_TEST_LIVE")
        override = GbifTestRunner.getenv("GBIF_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GbifTestRunner.getenv(key)
        ...
        explain = GbifTestRunner.getenv("GBIF_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GBIF_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GbifTestRunner._test_control is not None:
            return GbifTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GbifTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GbifTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GbifTestRunner.load_test_control()
...
def load_env_local():
    GbifTestRunner.load_env_local()


def env_override(m):
    return GbifTestRunner.env_override(m)


def entity_data(v):
    return GbifTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GbifTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GbifTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GbifTestRunner.load_test_control()


def live_client_options():
    return GbifTestRunner.live_client_options()


def live_delay_ms():
    return GbifTestRunner.live_delay_ms()
