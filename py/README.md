...
The Python SDK for the Gbif API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Enumeration(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Enumeration(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Gbif(None).load(
    {"id": "test01"}, None
)
