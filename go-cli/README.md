...
boru-driven command-line client **and** interactive REPL for the Gbif
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gbif-cli)
make build
...
./gbif-cli list enumeration
./gbif-cli load 1 enumeration            # {id:1} shorthand
./gbif-cli load '{id:1}' enumeration       # explicit match map
./gbif-cli list literature

# 5. Override the API base URL for a single call
GBIF_BASE=https://api.example.com ./gbif-cli list enumeration

# 6. No arguments -> interactive REPL
./gbif-cli
gbif> list enumeration
gbif> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gbif-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/gbif-cli list enumeration
   ```
...
```sh
./gbif-cli load 1 enumeration          # scalar shorthand for {id:1}
./gbif-cli load '{id:1}' enumeration     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `gbif>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `enumeration`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/gbif-cli`. |
...
(`./gbif-cli <expr>`) and interactively (the REPL), and expressions compose the
