// Command Gbif-cli is a CLI + REPL for the Gbif SDK,
...
const entitiesHelp = "enumeration literature occurrence registry species vocabulary"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GBIF_APIKEY (api key), GBIF_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GBIF_APIKEY carries the API key and
	// GBIF_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GBIF_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GBIF_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGbifSDK(opts)
