...
class RegistryDirectTest < Minitest::Test
  def test_direct_list_registry
    setup = registry_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-registry", setup[:live] ? "live" : "unit")
    if _should_skip
...
def registry_direct_setup(mockres)
