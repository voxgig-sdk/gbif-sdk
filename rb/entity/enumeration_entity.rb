...
class EnumerationEntity
  ...
  def make
    opts = @_entopts.dup
    EnumerationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GbifHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Enumeration, Hash] the current Enumeration data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GbifHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Enumeration fields)
  def match_get
  ...
  #
  # @param reqmatch [EnumerationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Enumeration.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Enumeration, Hash] the loaded Enumeration; raises GbifError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GbifHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [EnumerationListMatch, Hash, nil] match filter (any subset of
  #   Enumeration fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Enumeration>, Array] the matching Enumeration items; raises GbifError on failure
