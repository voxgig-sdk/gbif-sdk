...
class FtCtrl extends GbifControl
{
...
class FtClient
{
    ...
    private GbifUtility $utility;

    public function __construct(GbifUtility $utility, array $options)
    {
    ...
    public function get_utility(): GbifUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GbifContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GbifContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GbifConfig::make_feature($name);
    ...
    private static function buildUrl(GbifSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GbifError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GbifResponse($fetched);
        }
    ...
    private function fail(GbifContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GbifContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
