...
//    only - a typed GbifContext there would make every ctx
...
final class GbifOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GbifContext($clean, null);
    ...
    public static function err_from_map($m): ?GbifError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GbifError(is_string($code) ? $code : '', $msg);
    }
