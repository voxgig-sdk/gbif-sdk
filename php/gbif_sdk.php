...
class GbifSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = GbifHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = GbifHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!GbifFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, GbifFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return GbifUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = GbifHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = GbifHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = GbifHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = GbifHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new GbifError("graphql_error",
                "GbifSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
