...
class EnumerationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EnumerationEntity($this->_client, $opts);
    }

    /**
     * @param Enumeration|array $args Enumeration data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GbifHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Enumeration|array The current Enumeration data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Enumeration fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GbifHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Enumeration fields).
     */
    ...
    /**
     * Load a single Enumeration.
     *
     * @param EnumerationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EnumerationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Enumeration|array The loaded Enumeration as an assoc-array at the
     *   SDK boundary; throws GbifError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GbifHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Enumeration items matching the given filter.
     *
     * @param EnumerationListMatch|array|null $reqmatch Match filter (any subset
     *   of Enumeration fields) as an assoc-array; EnumerationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Enumeration[]|array A list of Enumeration items as assoc-arrays at
     *   the SDK boundary; throws GbifError on failure (item-5 convention).
     */
