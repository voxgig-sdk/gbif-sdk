...
class GbifNetsimFeature extends GbifBaseFeature
{
    ...
    private function _simulate(GbifContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(GbifContext $ctx, array $applied): void
    {
        $client = $this->client;
